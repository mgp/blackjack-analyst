...
/**
 * Allows creation of a table with players using basic strategy.
 * 
...
public class BasicStrategyTable {
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			System.err.println("Usage: java BasicStrategyTable [verbose] <num_rounds> "
					+ "<player name> <player name> ... <player name>");
			System.exit(1);
		}

		// create table with default dealer strategy
		Table basic_table = new Table("Basic Table", 6, DefaultDealer.getInstance(), 20, 200);
		ObserverList<TableObserver> table_obs = basic_table.getObservers();
		...
		for (int i = next_arg; i < args.length; ++i) {
			...
			Player new_player = new Player(new BasicStrategyPlayer(), player_name);
			basic_table.addPlayer(new_player);
			new_player.setBankroll(100000);
		}

		// play rounds of blackjack
		basic_table.playRounds(num_rounds);
		
		// print summary statistics
		System.out.println("\n" + wlto);
		for (Iterator<Player> i = basic_table.getPlayers().iterator(); i.hasNext(); ) {
